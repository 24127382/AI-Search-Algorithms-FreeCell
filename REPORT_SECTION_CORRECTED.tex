...
\subsection{Comparative Empirical Analysis}

\subsubsection{Metrics for Evaluation}

A fair comparison between BFS and DFS should report at least the following metrics:

\begin{table}[h]
\centering
\caption{BFS vs. DFS: Theoretical and Empirical Comparison}
\begin{tabular}{lcc}
\toprule
\textbf{Metric} & \textbf{BFS} & \textbf{DFS} \\
\midrule
Data Structure & FIFO queue & LIFO stack \\
Time Complexity & $O(b^d)$ & $O(b^d)$ in the worst case \\
Frontier Space & $O(b^d)$ & Typically much smaller frontier \\
Visited Space & Depends on explored states & Depends on explored states \\
Solution Optimality & Optimal under unit cost & Not guaranteed \\
Completeness & Yes on finite graphs with cycle handling & Yes on finite graphs with cycle handling \\
Measured Time & __ s & __ s \\
Measured Peak Memory & __ MB & __ MB \\
Expanded Nodes & __ & __ \\
Solution Length & __ moves & __ moves \\
\bottomrule
\end{tabular}
\end{table}
...
For the report, a line chart is preferable to an ASCII sketch because it can show how frontier size or memory pressure changes over time. A good figure caption would be:

\begin{quote}
\textit{Figure: Frontier size over time for BFS and DFS on the same FreeCell instances. BFS exhibits rapid growth, while DFS remains comparatively bounded in frontier size.}
\end{quote}

The main interpretation is expected to be:
\begin{itemize}
\item \textbf{BFS:} Frontier size increases rapidly as depth grows, which makes memory the limiting factor on difficult instances.
\item \textbf{DFS:} Frontier size grows much more slowly, but solution quality depends strongly on move ordering and may be far from optimal.
\end{itemize}
