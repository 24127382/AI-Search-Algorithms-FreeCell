...
from source.presentation.qt.control_panel import ControlPanel
...
class DealNumberDialog(QDialog):
    ...
    def _confirm(self):
        ...
        try:
            ...
            QMessageBox.warning(
                self, "Invalid Deal Number", "Deal number must be a valid integer."
            )
            return
        ...
        if dialog.selected_action == "back_previous":
            if not self.board.restore_previous_game_state():
                QMessageBox.information(
                    self,
                    "No Previous Game",
                    "No previous game state is available to restore.",
                )
