...
from source.presentation.qt.shared.qt import QColor, QMimeData, QPainter, QPixmap, Qt


def parse_drop_position(
    payload: str, target_position: tuple
...
def build_preview_pixmap(card, width: int, height: int) -> QPixmap:
    ...
    if not card_pixmap.isNull():
        ...
        painter.drawText(
            ...
            f"{card.rank} {SUIT_SYMBOL.get(card.suit, '?')}",
        )
...
def build_stacked_drag_pixmap(
    card_widget, drag_sequence: list, offset: int = 30
) -> QPixmap:
    ...
    for index, card in enumerate(cards):
        ...
        if index < len(cards) - 1:
            ...
            overlay.fillRect(
                0, 0, preview.width(), preview.height(), QColor(0, 0, 0, 30)
            )
            overlay.end()
    ...
    if parent is not None:
        for child in parent.children():
            if not hasattr(child, "position"):
                continue
            if (
                not isinstance(getattr(child, "position", None), tuple)
                or len(child.position) != 3
            ):
                continue
            if (
                child.position[0] == card_widget.position[0]
                and child.position[1] == card_widget.position[1]
            ):
