...
from source.domain.rule.rules import get_max_sequence_length, get_movable_sequences
from source.presentation.qt.board.constants import SLOT_TABLEAU
from source.presentation.qt.card.assets import SUIT_SYMBOL
from source.presentation.qt.shared.sound import play_invalid_move_sound
...
class BoardMoveCoreMixin:
    ...
    def _try_move(self, to_pos: tuple[str, int]):
        ...
        candidate_move = self._find_candidate_move(
            from_pos_engine, to_pos, expected_card
        )
        ...
        self.state = self.game_service.apply_move(self.state, candidate_move)
        self.move_count += 1
        self._set_source(None)
        self._emit_status(
    ...
    def _emit_sequence_limit_violation(
        self, to_pos: tuple[str, int], expected_card: Card | None
    ) -> bool:
        ...
        is_to_empty_tableau = (
            to_pos[0] == SLOT_TABLEAU and not self.state.tableau[to_pos[1]]
        )
    ...
    def _find_first_move(
        self, valid_moves: list, from_pos: tuple[str, int], target_type: str
    ):
