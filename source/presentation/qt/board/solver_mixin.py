...
from source.presentation.qt.board.solver_thread import SolverThread
from source.presentation.qt.card.assets import SUIT_SYMBOL
from source.presentation.qt.shared.env import env_int
from source.presentation.qt.shared.qt import (
    QDialog,
    QHBoxLayout,
    QLabel,
    QPushButton,
...
_SOLVER_REVIEW_PLAYBACK_INTERVAL_MS = env_int(
    "FRONTEND_SOLVER_PLAYBACK_INTERVAL_MS", 300, minimum=10
)
_SOLVER_MOVE_LIST_DIALOG_WIDTH = env_int(
    "FRONTEND_SOLVER_MOVE_LIST_DIALOG_WIDTH", 540, minimum=320
)
_SOLVER_MOVE_LIST_DIALOG_HEIGHT = env_int(
    "FRONTEND_SOLVER_MOVE_LIST_DIALOG_HEIGHT", 420, minimum=240
)
...
class BoardSolverMixin:
    ...
    def _emit_solver_interaction_locked(self):
        ...
        self._emit_status(
            "Solver is controlling the board. Press Stop to move cards manually."
        )
    ...
    def _update_solver_review_controls(self):
        ...
        if self._solver_prev_button is not None:
            self._solver_prev_button.setEnabled(
                (not self._solve_is_playing) and (not at_start)
            )
        if self._solver_next_button is not None:
            self._solver_next_button.setEnabled(
                (not self._solve_is_playing) and (not at_end)
            )
        if self._solver_play_pause_button is not None:
            self._solver_play_pause_button.setEnabled(
                move_count > 0 and (self._solve_is_playing or not at_end)
            )
    ...
    def _build_solver_states_from_path(self, path: list):
        ...
        for move in path:
            state = self.game_service.apply_move(state, move)
    ...
    def _on_solver_play_pause_clicked(self):
        ...
        self._update_solver_review_controls()
        self.solve_timer.start(_SOLVER_REVIEW_PLAYBACK_INTERVAL_MS)
        interval_seconds = _SOLVER_REVIEW_PLAYBACK_INTERVAL_MS / 1000.0
        self._emit_status(
            f"Solver playback running ({interval_seconds:.2f}s per move)."
        )
    ...
    def _on_solver_list_moves_clicked(self):
        ...
        dialog.resize(_SOLVER_MOVE_LIST_DIALOG_WIDTH, _SOLVER_MOVE_LIST_DIALOG_HEIGHT)
        ...
        for idx, move in enumerate(self._solve_moves, start=1):
            ...
            move_button.clicked.connect(
                lambda _, step=idx: self._jump_to_solver_move_from_dialog(step)
            )
    ...
    def _stop_solver_execution(self) -> bool:
        ...
        had_running_solver = (
            self.solver_thread is not None
            or self.is_solving
            or had_running_replay
            or had_running_review_mode
        )
    ...
    def solve_with_algo(self, algo: str):
        ...
        self.solver_thread.result_ready.connect(
            lambda path, label=solver_label, current_run_id=run_id: (
                self._on_solver_finished(label, path, current_run_id)
            )
        )
        self.solver_thread.error_occurred.connect(
            lambda error, label=solver_label, current_run_id=run_id: (
                self._on_solver_error(label, error, current_run_id)
            )
        )
        self.solver_thread.finished.connect(
            lambda current_run_id=run_id: self._on_solver_thread_finished(
                current_run_id
            )
        )
    ...
    def _on_solver_finished(self, algo, path, run_id: int):
        ...
        if run_id != self._active_solver_run_id:
            return
        elapsed_ms = (
            ((time.perf_counter() - self._solve_started_at) * 1000)
            if self._solve_started_at
            else 0.0
        )
        if path is None:
            self._emit_status(
                f"{algo} failed to find a solution after {elapsed_ms:.0f}ms."
            )
            return

        if path == []:
            self._emit_status(
                f"{algo} reports already solved after {elapsed_ms:.0f}ms."
            )
            return
    ...
    def _replay_next_solver_move(self):
        ...
        self.state = self.game_service.apply_move(self.state, move)
        self.move_count += 1
    ...
    def auto_to_foundation(self):
        ...
        for move in self.game_service.get_valid_moves(self.state, prune_safe=False):
            if move.to_pos[0] == "foundation":
                self._apply_automatic_move(
                    move, "Automatically sent 1 card to Foundation.", check_goal=True
                )
                return
