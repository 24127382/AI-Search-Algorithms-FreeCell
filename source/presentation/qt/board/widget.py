...
from source.presentation.qt.board.move_core_mixin import BoardMoveCoreMixin
from source.presentation.qt.board.move_interaction_mixin import BoardMoveInteractionMixin
from source.presentation.qt.board.solver_mixin import BoardSolverMixin
from source.presentation.qt.board.ui_layout_mixin import BoardUiLayoutMixin
from source.presentation.qt.board.ui_render_mixin import BoardUiRenderMixin
from source.presentation.qt.shared.qt import QWidget, Signal
from source.presentation.qt.shared.sound import play_win_sound


class BoardWidget(
    BoardUiRenderMixin,
    BoardUiLayoutMixin,
    BoardMoveInteractionMixin,
    BoardMoveCoreMixin,
    BoardSolverMixin,
    QWidget,
):
    ...
    def _build_initial_state(self) -> State:
        ...
        """
        deal_number, initial_state = self.game_service.build_initial_state(
            self.requested_deal_number
        )
    ...
    def new_game(self):
        ...
        if solver_stopped:
            self._emit_status(
                f"Solver stopped. New game started - Deal #{self.current_deal_number}."
            )
            return
    ...
    def _capture_pre_win_snapshot(
        ...
        if prev_state is None:
            if self.history:
                ...
                prev_history = (
                    list(self.history[:-1]) if prev_history is None else prev_history
                )
                prev_move_count = (
                    max(0, self.move_count - 1)
                    if prev_move_count is None
                    else prev_move_count
                )
