...
from source.presentation.qt.board.slot_widgets import SlotButton, TableauColumnWidget
...
class BoardUiLayoutMixin:
    ...
    def _build_bottom_info_row(self) -> QHBoxLayout:
        ...
        self._deal_number_label.setAlignment(
            Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter
        )
        bottom_row.addWidget(
            self._deal_number_label, alignment=Qt.AlignmentFlag.AlignLeft
        )
        ...
        self._solver_play_pause_button.clicked.connect(
            self._on_solver_play_pause_clicked
        )
        ...
        bottom_row.addWidget(
            self._solver_review_controls, alignment=Qt.AlignmentFlag.AlignRight
        )
        return bottom_row
    ...
    def _build_top_row(self) -> QHBoxLayout:
        ...
        for index in range(4):
            ...
            button.clicked.connect(
                lambda _, idx=index: self._on_foundation_clicked(idx)
            )
            button.drop_received.connect(self._on_drop_received)
    ...
    def _create_tableau_column_widget(self, col_idx: int) -> TableauColumnWidget:
        ...
        target_button.clicked.connect(
            lambda _, idx=col_idx: self._on_tableau_target_clicked(idx)
        )
