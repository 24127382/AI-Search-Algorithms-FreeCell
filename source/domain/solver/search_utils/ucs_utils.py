...
from source.domain.rule.rules import can_move_to_tableau
...
_UCS_FOUNDATION_MOVE_COST = env_int("UCS_FOUNDATION_MOVE_COST", -19, minimum=-10000)
_UCS_BASE_MOVE_COST = env_int("UCS_BASE_MOVE_COST", 6, minimum=0)
_UCS_GOOD_TABLEAU_BUILD_BONUS = env_int("UCS_GOOD_TABLEAU_BUILD_BONUS", 2, minimum=0)
_UCS_CREATE_EMPTY_COLUMN_BONUS = env_int("UCS_CREATE_EMPTY_COLUMN_BONUS", 2, minimum=0)
_UCS_BREAK_STACK_PENALTY = env_int("UCS_BREAK_STACK_PENALTY", 3, minimum=0)
_UCS_MEANINGLESS_EMPTY_FILL_PENALTY = env_int(
    "UCS_MEANINGLESS_EMPTY_FILL_PENALTY", 3, minimum=0
)
_UCS_RESTRUCTURE_PENALTY_CAP = env_int("UCS_RESTRUCTURE_PENALTY_CAP", 9, minimum=1)
_UCS_TO_FREECELL_PENALTY = env_int("UCS_TO_FREECELL_PENALTY", 2, minimum=0)
_UCS_FROM_FREECELL_TO_TABLEAU_BONUS = env_int(
    "UCS_FROM_FREECELL_TO_TABLEAU_BONUS", 1, minimum=0
)
...
def ucs_move_cost(move, prev_state=None, next_state=None):
    ...
    if move.to_pos[0] == "foundation":
        return _UCS_FOUNDATION_MOVE_COST
...
def ucs_forced_foundation_chain_cost(forced_move_count: int) -> int:
    """Return aggregate cost for a forced safe-foundation chain.

    Forced closure moves are always foundation moves, so UCS can use one
    cached unit cost and multiply by chain length instead of re-evaluating
    per-move edge cost.
    """
    if forced_move_count <= 0:
        return 0
    return forced_move_count * _UCS_FOUNDATION_MOVE_COST
